...
public class TestComplexOomage {
    ...
    public boolean haveNiceHashCodeSpread(Set<ComplexOomage> oomages) {
         ...
         */
        int[] set = new int [10];
        for(ComplexOomage iter: oomages){
            set[iter.hashCode()&0x7FFFFFFF % 10]+=1;
        }
        for (int i=0;i<10;i++){
            if(set[i]<oomages.size()/50||set[i]>oomages.size()/2.5) return false;
        }
        return true;
    }
    ...
    public void testWithDeadlyParams() {
        ...
        for(int i=0;i<1000;i++){
            ...
            for (int j = 0; j < N; j += 1) {
                if(j<5) params.add(255);
                else params.add(StdRandom.uniform(0, 255));
            }
            oomages.add( new ComplexOomage(params) );
        }
        //one more flaw is the last digit same, the hash will be same.
